...
![cover](https://s2.loli.net/2023/04/29/9ty4wZbPDBCchQg.webp)
...
这篇博客的目的是给出自定义VitePress博客可行的整套开发流程和注意事项。

这个博客项目参考了大量博主的文章和GitHub仓库的源代码，在这篇文章中我会使用外链表明我使用的解决方案参考了谁的博客、项目等。
...
A: 当时手里有域名也有服务器，想着自己搞个自定义博客，看VUE3的文档时发现了这个项目，查看文档后觉得很适合，于是选用。

Q: 需要掌握哪些技术才能自定义主题？

A: 最基本的CSS、HTML和JavaScript，以及一点点VUE，会用浏览器的`F12`开发者工具，其实这些都不会也没有关系，最重要的是会查阅文档、Google和Copy Code。
...
| [SM.MS](https://sm.ms/)                           | 免费的在线图床服务，加载速度不错，如果不想将图片保存在GitHub Repo |
...
填写目录名称时，`docs`是默认名，该目录在GitHub的代码占比分析中会被忽略，参考[这个](https://github.com/github-linguist/linguist/blob/master/docs/overrides.md#documentation)，如果你的Repo Languages显示不正常，应该创建`.gitattributes`在你的项目根目录(最外层)，添加类似以下的行：
...
对于文档而言这是必要的，但对博客而且必须去掉它来节省空间，从`confgi.js`中删除以下行以关闭它：
...
这是我唯一使用的markdown配置，且很有用，我查看了许多人的VitePress项目，发现他们都没有开启以下的markdown代码块行号显示选项，难道是因为这个隐藏在[文档](https://vitepress.dev/guide/markdown#line-numbers)的深处被忽略了？不管如何，我建议你在`config.js`中开启它：
...
我的博客完全是在明亮模式的基础上修改的，为了减少工作量，我只愿意开发一套主题，如果VitePress能锁定在dark mode，那将可以大大减少我的工作量。但是有一个可以参考的issue:[#1359](https://github.com/vuejs/vitepress/issues/1359)，这也许对你有用。
...
这个功能可以在VitePress构建完成后调用特定的JavaScript代码，很适合用来添加类似RSS feed和Site map(issue:[#520](https://github.com/vuejs/vitepress/issues/520))的功能。
...
我这个方法基本参考的尤雨溪`Vue Blog`中编写的[代码](https://github.com/vuejs/blog/blob/main/.vitepress/genFeed.ts)，但它使用了`'feed.rss'`文件名，这是错误的，应该使用`.xml`格式，否则RSS订阅文件将无法被浏览器正确显示。
...
图标可以从iconscout找，但VitePress Docs只给出了一种最鸡肋的SVG硬编码引用方式，但其实有更好的方法，参考issue:[#2290](https://github.com/vuejs/vitepress/issues/2290)。
...
新建立的文档和博客还没有内容来充实它，申请[Algolia DocSearch](https://docsearch.algolia.com/docs/what-is-docsearch)有点为难人了，不过VitePress就在alpha.66版本加入了`Local Search`功能，可以按照文档的说明简单快捷的使用它，只需要几行代码：
...
以前有`vitepress-plugin-search`插件用于支持本地搜索，我使用过该插件，效果上似乎差距不明显，但在样式上`Local Search`完胜对方，这很重要能减轻CSS工作量。

### Layout

`Layout`组件的`slot`是自定义VitePress博客的重中之重，就是因为有了`slot`才使VitePress在默认主题下页面组件仍有一定可拓展的空间，使用方法在[文档](https://vitepress.dev/guide/extending-default-theme#layout-slots)中已经写明，`Layout`一共有3种布局，我只使用了`home`和`doc`，没有使用`page`布局。
...
![image-20230430164526512](https://s2.loli.net/2023/04/30/bBUk9hIy8DpQaxG.webp)
...
![image-20230430164853343](https://s2.loli.net/2023/04/30/HCjTzVPk4hlvOo3.webp)
...
拉丁字体可以从[google-web-fonts-helper](https://gwfh.mranftl.com/)获取下载，但是中文字体我只能劝你别白费功夫了！

我建议还是用开头写的方法用`head`从网络导入Google Fonts，这要不了几行代码，但如果你执意用类似以下的方法加载字体：
...
而如果你使用相对路径，VitePress Console会不停输出以下信息，无比的烦人：
...
并且如果你还使用`github.io`子域，那么你的总花费将是0。

自定义域名非常简单，只需要两步：

1. 设置你的DNS解析记录一条A记录和AAAA记录指向GitHub的IPV4和IPV6地址，参考[文档](https://docs.github.com/en/pages/configuring-a-custom-domain-for-your-github-pages-site/managing-a-custom-domain-for-your-github-pages-site#configuring-an-apex-domain)；
...
为了让服务能在你的终端会话被关闭后仍然能够保持运行而不被杀死，需要`tmux`的帮助：
